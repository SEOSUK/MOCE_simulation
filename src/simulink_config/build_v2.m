...
algorithm(m,'Position Controller','v2_position',{'W_p_ref','W_p'},{'W_v_ref'},'CTRL',[360 100 535 190],true);
algorithm(m,'Velocity Controller','v2_velocity',{'W_v_ref','W_v','euler'},{'B_F_cmd'},'CTRL',[660 100 840 210],true);
algorithm(m,'Attitude Controller','v2_attitude',{'euler_ref','euler','B_omega'},{'B_omega_ref'},'CTRL',[360 360 535 470],true);
algorithm(m,'Rate Controller + DOB','v2_rate_dob',{'B_omega_ref','B_omega'}, ...
    {'B_tau_cmd','B_d_hat','B_tau_pid','B_tau_nom'},'CTRL',[660 360 840 470],true,2);
algorithm(m,'CoM Estimator','v2_moce',{'B_F_cmd','B_d_hat'}, ...
    {'B_c_hat','B_c_filtered'},'CTRL',[950 480 1120 565],true,1);
...
monitoring(m,[2050 510 2255 605]);
...
wire(m,'Rigid-Body Plant/1','Monitoring/1','');
wire(m,'Measurement Model/1','Monitoring/2','MeasuredStateBus');
...
for k=1:numel(inputs)
    ...
    if ~strcmp(inputs{k},'t')
        dim=3;
        if any(strcmp(inputs{k},{'thrust','servo_angle'})),dim=4;end
...
if isMotor,gain='PLANT.propeller.effectiveness';else,gain='PLANT.servo.dc_gain';end
add_block('simulink/Math Operations/Gain',[p '/Static effectiveness'],'Gain',gain,'Position',[495 30 570 70]);
wire(p,'Actuator state/1','Static effectiveness/1','');
if isMotor,from='Static effectiveness/1';
else
...
add_block('simulink/Continuous/Integrator',[r '/World velocity'],'InitialCondition','EXP.initial.velocity(:)','Position',[335 40 365 70]);
add_block('simulink/Continuous/Integrator',[r '/World position'],'InitialCondition','EXP.initial.position(:)','Position',[405 40 435 70]);
...
origins={'Translational Dynamics/1','Translational Dynamics/2','Kinematics/1','Rotational Dynamics/1','Actuator Wrench Model/1','Actuator Wrench Model/2'};
...
function monitoring(m,position)
subsystem(m,'Monitoring',{'TrueStateBus','MeasuredStateBus'},{},position);p=[m '/Monitoring'];
add_block('simulink/Sinks/Scope',[p '/True state'],'Position',[170 40 220 80]);
add_block('simulink/Sinks/Scope',[p '/Measured state'],'Position',[170 120 220 160]);
wire(p,'TrueStateBus/1','True state/1','');wire(p,'MeasuredStateBus/1','Measured state/1','');
a=Simulink.Annotation(p,sprintf(['Read-only diagnostics. All named control/actuator signals are logged to logsout.\n' ...
    'Use Simulation Data Inspector or out.logsout. PID/nominal torque and filtered CoM\n' ...
    'are logged inside their producing subsystems, with no diagnostic feedback.']));a.Position=[20 230];
end
